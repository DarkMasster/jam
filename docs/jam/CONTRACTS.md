...
- `LiberationSans SDF - Fallback.asset` работает в режиме `Dynamic`: TMP наращивает
  его атлас в Play Mode и резервирует `m_AtlasTextures` с пустым хвостом. В
  репозитории он хранится только чистым — один атлас, ноль глифов. Выросший вариант
...
- При отсутствии сохранённой настройки `jam.settings.locale` стартовым языком
  является английский (`en`). Сохранённый выбор пользователя имеет приоритет.
