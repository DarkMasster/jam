...
- Офисное управление разделяет движение и прицел: WASD/левый стик только двигают
  героя, `Office/Aim` получает курсор или правый стик, а поворот, автоподбор и
  бросок используют последнее валидное направление Aim. Runtime smoke этой правки
  отложен по прямому запросу продюсера; компиляция до остановки проверок прошла без
  ошибок.
