...
- `Prologue_Office` ещё не добавлена владельцем в Build Settings, поэтому
  `CharacterSelect` продолжает использовать `SampleScene`. Предмет, бросок и простое
  разрушение готовы, но Momentum, противник, урон герою, restart и финальный
  серверный босс остаются следующими срезами.
...
подключает `Prologue_Office` к Build Settings. Продюсер закрывает оставшиеся P0 из
`QUESTIONS.md`, включая №7–8 про набор противников.
