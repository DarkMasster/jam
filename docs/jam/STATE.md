...
- На ветке `polish/office-synty-art-pass` этот паттерн реализован и проверен на
  первом срезе: editor-слой `OfficeArtPass` подключает модели пака как visual
  children, подгоняет каждую под явно указанный greybox-объём, выключает
  vendor-коллайдеры и переводит материалы пака в офисную палитру project-owned
  material variants `M_Synty_*.mat`. Заменены стартовый кабинет, пара столов и
  кресел open space на `z = -15`, левая секция стекла переговорной и две серверные
  стойки. Коллайдеры, маршрут, триггеры и `Assets/PolygonOffice` не изменились.
...
- Art-pass POLYGON Office сделан только на проверочном срезе: остальные столы,
  кресла, правая секция стекла, серверная, рецепция, интерактивные предметы,
  противники и стойки босса пока greybox. Отдельно не решён вопрос
...
Следующий офисный срез art-pass распространяет `OfficeArtPass` на остальной open
space, переговорную, серверную и рецепцию по тем же правилам.
