...
- Текущая контрольная точка: `M8` расширенная разрушаемость завершена; основной
  следующий срез — остаток `POLYGON Office art-pass` (оболочка и интерактив)
...
| Управление Aim | Done, runtime-проверка пропущена | `Move` больше не разворачивает героя; `Office/Aim` принимает позицию курсора и направление правого стика, а подбор и бросок используют единое последнее направление прицела | Проверить мышь и геймпад в следующем общем Play Mode smoke; до этого не менять контракт `Aim` |
...
| `M8` Расширенная разрушаемость | Done | `D1`–`D4`: 32 новых объекта плюс 4 принтера имеют `Intact → Broken`, per-object Momentum, DNP и restart. Broken-состояние отключает 64 принадлежащих объектам коллайдера; герой ломает объекты броском или тараном от накопленной скорости | Сохранить как стабильную основу; art-pass не должен переносить broken-state в vendor-каталог |
...
### M8 — Расширенная разрушаемость маршрута — Done / P1

До `M8` разрушаемых объектов было ровно четыре — принтеры. Теперь офис по всему
маршруту ломается под броском или тараном, а не остаётся неподвижной декорацией, при
этом граница «разрушение ограничено подготовленными состояниями, без свободной
фрагментации» остаётся в силе. Каждый новый объект получает ту же пару состояний
`Intact → Broken` и тот же контракт `IOfficeImpactTarget`, что и принтер.

Критерии приёмки:

- [x] Каждый объект из слайсов `D1`–`D4` переходит `Intact → Broken` от броска или
      достаточно быстрого тарана героя и
      даёт однозначный визуальный feedback.
- [x] Разрушение любого нового объекта повышает Momentum и показывает `СЛОМАНО`
      через существующий `IGameFeedback`, без нового presentation-слоя.
- [x] Слабый, повторный и не засчитанный удар по-прежнему не даёт ни смены
      состояния, ни popup.
- [x] Broken-состояние становится проходимым, включая стекло переговорной; restart
      возвращает все коллайдеры и объекты в `Intact`.
- [x] Разрушение не затрагивает `Access Hold`, внешние стены, стойки и кольцо босса;
      новый проход через стекло является явным решением продюсера.
- [x] Босс, `OfficeItemGuarantee`, Reflection beat и подсказки `OfficeCoach` не
      регрессируют.

#### Объём по слайсам

Слайсы берутся по порядку: следующий начинается только после того, как предыдущий
проверен и записан в историю изменений.

| Слайс | Объекты | Количество | Почему в этом порядке |
|---|---|---|---|
| `D1` Мониторы | `Monitor` + `Monitor Glow` внутри `Desk` маршрута | 7 (Start Desk + 6 столов open space) | Самый дешёвый: собственного коллайдера у монитора нет, проходимость и line of sight не меняются вовсе |
| `D2` Серверные стойки | `Server Rack {x} {z}` серверной | 12 (6 позиций `x` × 2 ряда `z`) | Плотная красная зона, самый заметный эффект. Требует разделения prefab — см. ограничение 1 |
| `D3` Столы, шкафы и рецепция | `Start Desk`, `Open Desk L/R` (6), `Start Cabinet L/R`, `Reception Desk L/R` | 11 | Меняет проходимость и line of sight, поэтому идёт после проверенных `D1`–`D2` |
| `D4` Стекло переговорной | `Meeting Glass Left/Right` | 2 | Завершён по прямому решению продюсера: broken-состояние открывает проход, правый vendor-visual подключит будущий остаток `A3` |

Итог: `32` новых разрушаемых объекта поверх четырёх принтеров. Фоновый ряд
`Distant Desk` (12 столов) в срез не входит: он стоит за стенами зала, у него нет
коллайдеров и попасть в него броском нельзя. `Boss Rack 01..12` не входит никогда —
стойками босса владеет `OfficeBossEncounter`.

#### Найденные ограничения

1. **Общий prefab `ServerRack` — закрыто.** `OfficeSceneBuilder` раньше брал один и тот же prefab и
   для двенадцати стоек серверной, и для двенадцати стоек босса
   (`GetOrCreatePrefab("ServerRack", …)`). Компонент `OfficeBreakable`, добавленный
   в шаблон, попал бы на стойки босса и позволил бы сломать их до сборки. `D2`
   теперь использует отдельный `ServerRack_Breakable`; стойки босса остаются на
   `ServerRack` без `OfficeBreakable`. Отдельно проверен
   `OfficeBossEncounter.SetRackColliders`: он собирает коллайдеры через
   `GetComponentsInChildren`, поэтому любой лишний коллайдер в иерархии стойки
   включается обратно во время боя.
2. **Line of sight противников — принятое изменение.** `OfficeChaser.IsLineOfSightBlocked` считает
   блокирующим любой не-триггерный коллайдер, кроме игрока и carryable. Принтер
   после разрушения снижает свой `BoxCollider` до 40% высоты; та же схема на стойке,
   столе или стойке рецепции откроет креслу сектор обзора, которого не было при
   проектировании маршрута. По решению продюсера broken-состояние всей мебели
   проходимо, поэтому сектор обзора после разрушения намеренно может расшириться.
3. **Стекло переговорной — закрыто решением продюсера.** Секции
   `Meeting Glass Left/Right` (`0.18 × 2.5 × 13`) держат стену переговорной. Если
   разрушение убирает коллайдер, в авторском маршруте появляется новый проход и
   критерий `M2` про читаемые границы зон перестаёт выполняться. Кроме того
   `OfficeReflectionBeat` показывает силуэт именно за правым стеклом. Продюсер
   разрешил новый проход; Reflection beat остаётся на прежнем триггере и не
   блокирует разрушение стекла.
4. **Насыщение Momentum — закрыто.** `OfficeMomentum.breakGain` — `0.26`, шкала заполняется с
   четырёх разрушений. При `36` целях Momentum перестаёт быть ресурсом темпа и
   становится фоном. Введён per-object вклад: монитор `0.08`, мебель `0.14`,
   стойка `0.20`, стекло `0.24`, принтер `0.26`.
5. **Счётчик HUD — закрыто.** `RegisterBreakableTarget` ведёт общий `X/Y`, и строка
   `РАЗРУШЕНО: … • 1/36` читается как невыполнимая задача вместо награды за темп.
   Нужен флаг «объект считается целью»: цели — принтеры и стойки, остальное даёт
   Momentum и popup, но не двигает счётчик. Фактический итог — `X/16`.
6. **Стыковка с art-pass — закрыто для текущих слайсов.** Broken-состояния — project-owned геометрия, art-pass её
   не трогает (см. «Объекты, которые art-pass не трогает»). Но `Status LED` стоек
   вынесены прямыми детьми корня, а не в группу `Intact`, поэтому `D2`
   реструктурировал шаблон стойки; `Synty Visual` входит в intact-владение
   разрушаемой стойки, а art-pass пропускает все группы `Broken`.

#### Решения продюсера

1. HUD считает только принтеры и серверные стойки: `16` целей из `36` объектов.
2. Все broken-состояния проходимы, включая стекло переговорной; новый проход
   является допустимым изменением маршрута.
3. Таран повторяет веб-демо: реальная planar velocity героя умножается на
   `1 + Momentum × 0.4`; успешный таран сохраняет `90%` скорости.
4. Вклад в Momentum: монитор `0.08`, стол/шкаф/рецепция `0.14`, стойка `0.20`,
   стекло `0.24`, принтер `0.26`.

#### Правила слайса

- Используются существующие `OfficeBreakable`, `IOfficeImpactTarget`,
  `OfficeRunReset`, `OfficeFeedback` и DNP-preset `M7`; `OfficePlayerController`
  передаёт накопленную скорость столкновения в тот же impact-контракт.
- Порог `breakImpactSpeed` задаётся на объекте, а не глобально: крупная мебель
  требует более сильного броска, чем монитор.
- Новые `displayName` получают RU/EN ключи в таблице `Office` — строка статуса идёт
  через `LocalizeRuntimeName`.
- Свободной фрагментации, физики обломков и новых Rigidbody нет: broken-state — это
  заранее собранная геометрия, как у принтера.
- Слайс закрыт, когда подтверждены: смена состояния и отсутствие ложных срабатываний
  по каждому объекту, проходимость broken-состояний, восстановление всех коллайдеров,
  доступность босса, scene validation `0 issues` и Console без игровых ошибок.

## Зависимости и владельцы

| Решение или ресурс | Владелец | Нужно для |
|---|---|---|
| Карта `Office` в общем input asset | Интегратор | Закрыто: карта `Office` (Move, Aim, Primary, Secondary, Interact) добавлена; офис читает `Move`, `Aim` и `Primary`, причём `Aim` имеет тип `PassThrough` для независимых событий курсора и правого стика |
...
| Счётчик целей HUD, судьба стекла и вклад в Momentum | Продюсер | Закрыто в `M8`: `16` целей HUD, все broken проходимы, per-object Momentum и таран по скорости веб-демо |
...
2. Слайсы разрушаемости `M8` с конца: `D4` стекло, затем `D3` столы и шкафы, затем
   `D2` стойки. `D1` мониторы стоит дешевле всего и режется последним.
3. Призрачное эхо Reflection; оставить визуальный beat у переговорной.
4. Вариации атак, реплик, декора и эффектов.
5. Дополнительные комнаты за пределами обязательного маршрута.
...
Срез `M8` завершён параллельно art-pass. Следующие art-слайсы обязаны сохранять
project-owned `Broken`, владение vendor-визуалами и правило отключения коллайдеров;
повторно проектировать разрушаемость в `A3`/`A7` не нужно.
...
|---|---|---|---|
| 2026-08-02 | `feature/office-aim-controls` | Поворот героя отделён от `Move`: `OfficePlayerController` читает `Office/Aim`, преобразует курсор в направление на плоскости XZ и правый стик в camera-relative направление, сохраняя последний валидный Aim. Автоподбор и бросок используют то же направление. `Aim` переведён из `Value` в `PassThrough`, чтобы позиция курсора не подавляла события стика | Unity скомпилировала изменения без ошибок до указания остановить проверки; `git diff --check` пройден. Play Mode и Unity Test Framework пропущены по прямому запросу продюсера, поэтому mouse/gamepad runtime smoke остаётся для следующего общего прогона |
...
| 2026-08-02 | `feature/office-m8-destructibility` / `M8` | Реализованы `D1`–`D4`: 7 мониторов, 12 стоек серверной, 7 столов, 2 шкафа, 2 стойки рецепции и 2 секции стекла. Добавлены per-object Momentum, `X/16`, intact/broken visual ownership, проходимые broken-коллайдеры и таран героя по формуле веб-демо `speed × (1 + Momentum × 0.4)`. `ServerRack_Breakable` отделён от стоек босса; исправлен повторный rebuild prefab внутри циклов, который сбрасывал ранние instance в `(0,0,0)` | Compile и script validation — 0 errors; scene validation — 0 issues; полный доступный EditMode-набор `1/1 passed`. Edit Mode scene metrics: `36` breakable, `16` objectives, `171/171` colliders, vendor colliders `0`, boss breakables `0`, desk colliders `6`, monitor bounds `1.25×0.72×0.08`, misplaced loop instance `0`. Controlled Play Mode: weak/strong/repeat `false/true/false`; все `36` broken, active broken-state `36`, runtime colliders `159→95→159`, reset broken `0`; external cabinet visuals `2→0`; стекло пробито тараном `9.5` и пройдено (`x 7.00→7.42→8.62`); стойка выдержала базовый таран и сломалась при Momentum (`false→true`). Console без игровых ошибок; TMP fallback importer warning после очистки, refresh и повторной перерисовки не воспроизводится |
| 2026-08-02 | `polish/office-synty-art-pass` / `M8` | По запросу продюсера зафиксирован срез расширенной разрушаемости `M8`: 32 новых объекта в слайсах `D1` мониторы (7), `D2` серверные стойки (12), `D3` столы, шкафы и рецепция (11), `D4` стекло переговорной (2), плюс критерии приёмки, правила слайса, порядок сокращения и строка в таблице зависимостей. Записаны шесть найденных ограничений и три открытых вопроса продюсеру, блокирующих старт. Runtime и сцена не изменялись | Опись и ограничения сняты с кода, а не с памяти: разрушаемых объектов сейчас `4` (`BuildBreakables` — четыре принтера); `GetOrCreatePrefab("ServerRack", …)` используется и в серверной, и в `Boss Racks`, поэтому компонент в шаблоне попал бы на стойки босса; `OfficeBossEncounter.SetRackColliders` собирает коллайдеры через `GetComponentsInChildren`; `OfficeChaser.IsLineOfSightBlocked` блокируется любым не-триггерным коллайдером, кроме игрока и carryable, а `OfficeBreakable.ApplyState` снижает `BoxCollider` до 40% высоты; `OfficeMomentum.breakGain` = `0.26`; `Status LED` стоек — прямые дети корня, а не группы `Intact`; фоновые `Distant Desk` создаются с `colliders = false`. Подсчёт объектов сверен с `OfficeSceneBuilder`: 3 пода × 2 стола open space + `Start Desk` = 7, `rackX` 6 × 2 ряда = 12, 2 шкафа, 2 стойки рецепции, 2 секции стекла |
